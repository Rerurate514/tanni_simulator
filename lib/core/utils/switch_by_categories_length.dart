...
T switchByCategoriesLength<T>(
  ...
  T Function(RequirementCategoryModel category, CategoryType categoryType)
  onOneCategory,
  T Function(RequirementCategoryModel prof, RequirementCategoryModel gen)
  onTwoCategories,
) {
  ...
  return switch (categories.length) {
    ...
    _ => () {
      final prof = categories.firstWhere(
        (c) => c.category == CategoryType.professional,
      );
      final gen = categories.firstWhere(
        (c) => c.category == CategoryType.general,
      );
